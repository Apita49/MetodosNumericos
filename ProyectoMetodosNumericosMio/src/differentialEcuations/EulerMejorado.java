...
public class EulerMejorado {
	...
	public static String eulerMejorado(String f, String h, String x0,
	...
	public static void eulerMejoradoForPDF(String f, String h, String x0,
			String y0, String eval, boolean iteraciones, String title) {
		String content = "";
		try {
			content = ansToStringForPDF(
					eulerMejorado(f, new Double(h), new Double(x0), new Double(
							y0), new Integer(eval)), f, h, x0, y0, eval,
					iteraciones);
		} catch (NumberFormatException | ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public static List<List<Double>> eulerMejorado(String f, double h,
			double x0, double y0, double eval) throws ParseException {
		...
		double a = 0;
		try {
			a = (double) j.evaluate(func);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		j.addVariable("x", x0 + h);
		j.addVariable("y", y0 + (h * a));
		double b = 0;
		try {
			b = (double) j.evaluate(func);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		double yActual = y0 + (h / 2) * (a + b);
		ans.get(0).add(x0 + h);
	...
	public static String eulerMejoradoSeg(String f, String g, String h,
			String n, String x0, String y0, String t0, boolean iteraciones) {
		try {
			return "<html>"
					+ ansToString2ndoOrden(
							eulerMejoradoSeg(f, g, new Double(h),
	...
	public static void eulerMejoradoSegForPDF(String f, String g, String h,
			String n, String x0, String y0, String t0, boolean iteraciones,
			String title) {
		String content = "";
		try {
			content = ansToString2ndoOrdenForPDF(
					eulerMejoradoSeg(f, g, new Double(h), new Double(x0),
	...
	public static double[][] eulerMejoradoSeg(String f, String g, double h,
			double z0, double y0, double t0, double eval) throws ParseException {
		...
		for (double i = t0; pos < ans[0].length; i += h) {
			...
			try {
				double gEval = (double) j.evaluate(G);
				double fEval = (double) j.evaluate(F);
				j.addVariable("x", zActual + (h * (fEval)));
				j.addVariable("y", yActual + (h * gEval));
				j.addVariable("t", i + h);
				double gEvalLargo = (double) j.evaluate(G);
				double fEvalLargo = (double) j.evaluate(F);
				yActual = yActual + (h / 2) * (gEval + gEvalLargo);
				zActual = zActual + (h / 2) * (fEval + fEvalLargo);
	...
	public static void main(String[] args) {
		try {
			double[][] b = eulerMejoradoSeg("cos(t)-4y", "x", 0.1, 0, 0, 0, 1);
		...
		// try {
		// List<List<Double>> a = eulerMejorado("x-y", 0.2, 0, 2, 1);
		// System.out.println();
		// } catch (ParseException e) {
		// // TODO Auto-generated catch block
		// e.printStackTrace();
