...
public class EulerModificado {
	...
	public static String eulerModificado(String f, String h, String x0,
	...
	public static void eulerModificadoForPDF(String f, String h, String x0,
			String y0, String eval, boolean iteraciones, String title) {
		String content = "";
		try {
			content = ansToStringForPDF(
					eulerModificado(f, new Double(h), new Double(x0),
							new Double(y0), new Integer(eval)), f, h, x0, y0,
					eval, iteraciones);
		} catch (NumberFormatException | ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public static String ansToStringForPDF(List<List<Double>> ans, String f,
		...
		int maxLengths = maxLength(ans);
		String answer = "Euler Modificado\n";
	...
	public static List<List<Double>> eulerModificado(String f, double h,
		...
		j.addVariable("x", x0 + h / 2);
		j.addVariable("y", y0 + (h / 2 * a));
		double b = 0;
		try {
		...
		for (double i = x0 + h; i < eval * h; i += h) {
			...
			j.addVariable("x", i + h / 2);
			j.addVariable("y", yActual + (h / 2 * a));
			try {
				b = (double) j.evaluate(func);
			} catch (Exception e) {
				// TODO Auto-generated catch block
	...
	public static String eulerModificadoSeg(String f, String g, String h,
			String n, String x0, String y0, String t0, boolean iteraciones) {
		try {
			return "<html>"
					+ ansToString2ndoOrden(
							eulerModificadoSeg(f, g, new Double(h), new Double(
	...
	public static void eulerModificadoSegForPDF(String f, String g, String h,
			String n, String x0, String y0, String t0, boolean iteraciones,
			String title) {
		String content = "";
		try {
			content = ansToString2ndoOrdenForPDF(
					eulerModificadoSeg(f, g, new Double(h), new Double(x0),
	...
	public static double[][] eulerModificadoSeg(String f, String g, double h,
		...
		for (double i = t0; pos < ans[0].length; i += h) {
			...
			try {
				...
				j.addVariable("x", zActual + ((h / 2) * (fEval)));
				j.addVariable("y", yActual + ((h / 2) * gEval));
				j.addVariable("t", i + h);
				double gEvalLargo = (double) j.evaluate(G);
				double fEvalLargo = (double) j.evaluate(F);
				yActual = yActual + (h) * (gEvalLargo);
				zActual = zActual + (h) * (fEvalLargo);
	...
	public static void main(String[] args) {
		try {
			double[][] a = eulerModificadoSeg("cos(t)-4y", "x", 0.1, 0, 0, 0, 1);
		...
		// try {
		// List<List<Double>> a = eulerModificado("x-y", 0.2, 0, 2, 1);
		// System.out.println();
		// } catch (ParseException e) {
		// // TODO Auto-generated catch block
		// e.printStackTrace();
